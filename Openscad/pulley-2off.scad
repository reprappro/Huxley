...
pulley_b_dia = 11.5;	// pulley base diameter, standard = 20
...
module base() {
			...
			{
				cylinder(r=pulley_b_dia/2,h=pulley_b_ht*2,$fn=pulley_b_dia*2, $fn=32);
				translate([0,0,pulley_b_ht*2]) cylinder(r1=pulley_b_dia/2, r2=0, h=pulley_b_dia/2, $fn=32);
			}
}
	...
	difference()
	 {	 
		union()
		{
			...
			if(top_flange)
			{
			  mirror([0,0,1]) translate([0,0,-(pulley_t_ht+pulley_b_ht)]) base();
/*				translate([0,0,pulley_t_ht])
		...
		difference()
		{
			cylinder(r=motor_shaft/2,h=pulley_b_ht + pulley_t_ht + retainer_ht + 2,
				$fn=motor_shaft*8);
			translate([motor_shaft/2 + shaft_flat ,0,(pulley_b_ht + pulley_t_ht)/2+1])
				cube([motor_shaft,3*motor_shaft,pulley_b_ht + pulley_t_ht], 
					center=true);
		}
		translate([0,0,pulley_b_ht + pulley_t_ht-5]) cylinder(r1=motor_shaft/2-0.5,r2=motor_shaft/2+0,h=5.1,$fn=32);
